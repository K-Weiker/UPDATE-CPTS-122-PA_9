...
void Game::normal(int xStart, int yStart, int xEnd, int yEnd)
{
    ...
    mHandler->undoPieceRender(xStart, yStart);
    mHandler->undoPieceRender(xEnd, yEnd);
    movingPiece->render();
...
void Game::enPassant(int xStart, int yStart, int xEnd, int yEnd)
{
    ...
    mHandler->undoPieceRender(xStart, yStart);
    mHandler->undoPieceRender(xEnd, capturedY);
    movingPawn->render();
}
...
{
    SDL_Texture* text_rook = mHandler->loadImage("res/white-rook4.png");
    SDL_Texture* text_knight = mHandler->loadImage("res/white-nightrd.png");
    SDL_Texture* text_bishop = mHandler->loadImage("res/white-bishop.png");
    SDL_Texture* text_queen = mHandler->loadImage("res/white-grassh.png");
    int y_draw = 0;
    Piece::Team team = Piece::WHITE;

    if (m_field[xStart][yStart]->getTeam() == Piece::BLACK)
    {
        text_rook = mHandler->loadImage("res/black-rook4.png");
        text_knight = mHandler->loadImage("res/black-nightrd.png");
        text_bishop = mHandler->loadImage("res/black-bishop.png");
        text_queen = mHandler->loadImage("res/black-grassh.png");
        y_draw = 3 * mHandler->SCREEN_HEIGHT / 4;
        team = Piece::BLACK;
    }

    SDL_SetRenderDrawColor(mHandler->m_renderer, 155, 103, 60, 255);
    SDL_Rect rectangle = { 0,
                          y_draw,
                          mHandler->SCREEN_WIDTH / 4,
                          mHandler->SCREEN_HEIGHT / 4 };
    SDL_RenderFillRect(mHandler->m_renderer, &rectangle);
    SDL_Rect src = { 0, 0, 60, 60 };
    mHandler->DrawRectangle(src, rectangle, text_rook);

    SDL_SetRenderDrawColor(mHandler->m_renderer, 255, 255, 255, 255);
    rectangle.x = mHandler->SCREEN_WIDTH / 4;
    SDL_RenderFillRect(mHandler->m_renderer, &rectangle);
    mHandler->DrawRectangle(src, rectangle, text_knight);

    SDL_SetRenderDrawColor(mHandler->m_renderer, 155, 103, 60, 255);
    rectangle.x = 2 * mHandler->SCREEN_WIDTH / 4;
    SDL_RenderFillRect(mHandler->m_renderer, &rectangle);
    mHandler->DrawRectangle(src, rectangle, text_bishop);

    SDL_SetRenderDrawColor(mHandler->m_renderer, 255, 255, 255, 255);
    rectangle.x = 3 * mHandler->SCREEN_WIDTH / 4;
    SDL_RenderFillRect(mHandler->m_renderer, &rectangle);
    mHandler->DrawRectangle(src, rectangle, text_queen);
    ...
    while (quit == false)
    {
        while (SDL_PollEvent(&mHandler->m_event))
        {
            if (mHandler->m_event.type == SDL_QUIT)
            {
                quit = true;
            }

            if (mHandler->m_event.type == SDL_MOUSEBUTTONDOWN)
            {
                x = mHandler->m_event.button.x / 160;
                y = mHandler->m_event.button.y / 160;

                if (y >= y_draw / 160 && y < y_draw / 160 + 1)
                {
                    if (x < mHandler->SCREEN_WIDTH / 640)
                    {
                        clickedOn = new Rook(team, std::pair<int, int>(xEnd, yEnd), mHandler);
                    }
                    else if (x < 2 * mHandler->SCREEN_WIDTH / 640)
                    {
                        clickedOn = new Knight(team, std::pair<int, int>(xEnd, yEnd), mHandler);
                    }
                    else if (x < 3 * mHandler->SCREEN_WIDTH / 640)
                    {
                        clickedOn = new Bishop(team, std::pair<int, int>(xEnd, yEnd), mHandler);
                    }
                    else if (x <= 4 * mHandler->SCREEN_WIDTH / 640)
                    {
                        clickedOn = new Queen(team, std::pair<int, int>(xEnd, yEnd), mHandler);
                    }
                    std::cout << x << " " << mHandler->SCREEN_WIDTH / 640 << std::endl;
                }
            }

            if (mHandler->m_event.type == SDL_MOUSEBUTTONUP && clickedOn != nullptr)
            {
                quit = true;
            }
        }
...
void Game::castles(int xStart, int yStart, int xEnd, int yEnd)
{
    ...
    if (xEnd == 0) {
        ...
        mHandler->undoPieceRender(4, yEnd);
        mHandler->undoPieceRender(0, yEnd);
    ...
    // Kingside castling (rook on h-file)
        ...
        mHandler->undoPieceRender(4, yEnd);
        mHandler->undoPieceRender(7, yEnd);
...
void Game::showPossMoves(Piece* piece)
{
    ...
    for (const auto& move : possible)
    {
        ...
        if ((x % 2 == 0 && y % 2 == 1) || (x % 2 == 1 && y % 2 == 0)) {
            SDL_SetRenderDrawColor(mHandler->m_renderer, 0, 134, 139, 255); // darker
        }
        else {
            SDL_SetRenderDrawColor(mHandler->m_renderer, 164, 211, 238, 255); // lighter
        }

        rectangle = {
            x * mHandler->SCREEN_WIDTH / 8,
            y * mHandler->SCREEN_HEIGHT / 8,
...
void Game::undoshowPossMoves(Piece* piece) // to get rid of highlighted poss move
{
    ...
    for (const auto& value : possible) {
        ...
        SDL_Rect rectangle = { std::get<0>(value) * mHandler->SCREEN_WIDTH / 8,
                                  std::get<1>(value) * mHandler->SCREEN_HEIGHT / 8,
                                  mHandler->SCREEN_WIDTH / 8,
                                  mHandler->SCREEN_HEIGHT / 8 };
        SDL_RenderFillRect(mHandler->m_renderer, &rectangle);
